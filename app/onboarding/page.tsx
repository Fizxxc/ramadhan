"use client";

import React from "react";
import { supabase } from "@/lib/supabase/client";
import { useRouter } from "next/navigation";

type Step = 1 | 2 | 3;

export default function OnboardingPage() {
  const router = useRouter();

  const [step, setStep] = React.useState<Step>(1);
  const [name, setName] = React.useState("");
  const [tilawahTarget, setTilawahTarget] = React.useState(1);
  const [hafalanTarget, setHafalanTarget] = React.useState(1);

  // 🔽 UPDATE
  const [ramadhanDay, setRamadhanDay] = React.useState(1);
  const [notifImsak, setNotifImsak] = React.useState(true);
  const [notifMaghrib, setNotifMaghrib] = React.useState(true);

  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState("");

  const next = () => setStep((s) => (s === 1 ? 2 : s === 2 ? 3 : 3));
  const back = () => setStep((s) => (s === 3 ? 2 : 1));

  const finish = async () => {
    setError("");
    if (!name.trim()) return setError("Nama panggilan wajib diisi");

    setLoading(true);

    const { data: sess } = await supabase.auth.getSession();
    const uid = sess.session?.user.id;
    if (!uid) {
      setLoading(false);
      return setError("Sesi tidak ditemukan. Silakan login ulang.");
    }

    // 🔽 preferences LENGKAP & KOMPATIBEL
    const preferences = {
      target_tilawah: tilawahTarget,
      target_hafalan: hafalanTarget,
      ramadhan_day: ramadhanDay,
      notif: {
        imsak: notifImsak,
        maghrib: notifMaghrib,
      },
      active_location: "rumah",
      locations: {
        rumah: null,
        kantor: null,
      },
    };

    const { error: err } = await supabase.from("profiles").upsert({
      id: uid,
      name: name.trim(),
      preferences,
    });

    // init quran_progress
    await supabase
      .from("quran_progress")
      .upsert({ user_id: uid }, { onConflict: "user_id" });

    setLoading(false);
    if (err) return setError(err.message);

    router.replace("/app/home");
  };

  return (
    <main className="min-h-dvh bg-white px-5 pt-10">
      <div className="mx-auto max-w-[430px]">
        <div className="flex items-center justify-between">
          <div className="text-sm font-semibold text-text">Onboarding</div>
          <div className="text-xs text-muted">Langkah {step}/3</div>
        </div>

        {error && (
          <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
            {error}
          </div>
        )}

        {/* STEP 1 */}
        {step === 1 && (
          <section className="mt-6">
            <h1 className="text-xl font-semibold text-text">Kenalan dulu</h1>
            <p className="mt-1 text-sm text-muted">
              Isi nama panggilan agar terasa lebih personal.
            </p>

            <label className="mt-5 block">
              <span className="text-sm font-medium text-text">
                Nama panggilan
              </span>
              <input
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="mt-2 w-full rounded-2xl border border-border bg-white px-4 py-3 text-text outline-none focus:ring-2 focus:ring-emerald-200"
                placeholder="Contoh: Fulan"
              />
            </label>

            <button
              onClick={next}
              className="mt-6 w-full rounded-2xl px-4 py-3 text-white font-semibold active:scale-[0.98] transition"
              style={{
                background:
                  "linear-gradient(135deg, rgb(16 185 129), rgb(59 130 246))",
              }}
            >
              Lanjut
            </button>
          </section>
        )}

        {/* STEP 2 */}
        {step === 2 && (
          <section className="mt-6">
            <h1 className="text-xl font-semibold text-text">
              Target Ramadhan
            </h1>
            <p className="mt-1 text-sm text-muted">
              Target kecil tapi konsisten itu baik.
            </p>

            <div className="mt-5 rounded-3xl border border-border bg-white p-4">
              <div className="text-sm font-semibold text-text">
                Target tilawah
              </div>
              <p className="mt-1 text-xs text-muted">Halaman / juz.</p>
              <input
                type="number"
                min={1}
                value={tilawahTarget}
                onChange={(e) =>
                  setTilawahTarget(Math.max(1, Number(e.target.value)))
                }
                className="mt-3 w-full rounded-2xl border border-border bg-white px-4 py-3"
              />
            </div>

            <div className="mt-3 rounded-3xl border border-border bg-white p-4">
              <div className="text-sm font-semibold text-text">
                Target hafalan
              </div>
              <p className="mt-1 text-xs text-muted">Ayat per hari.</p>
              <input
                type="number"
                min={1}
                value={hafalanTarget}
                onChange={(e) =>
                  setHafalanTarget(Math.max(1, Number(e.target.value)))
                }
                className="mt-3 w-full rounded-2xl border border-border bg-white px-4 py-3"
              />
            </div>

            <div className="mt-6 grid grid-cols-2 gap-2">
              <button
                onClick={back}
                className="rounded-2xl border border-border bg-white px-4 py-3 text-sm font-semibold"
              >
                Kembali
              </button>
              <button
                onClick={next}
                className="rounded-2xl px-4 py-3 text-sm font-semibold text-white"
                style={{
                  background:
                    "linear-gradient(135deg, rgb(16 185 129), rgb(59 130 246))",
                }}
              >
                Lanjut
              </button>
            </div>
          </section>
        )}

        {/* STEP 3 */}
        {step === 3 && (
          <section className="mt-6">
            <h1 className="text-xl font-semibold text-text">Preferensi</h1>
            <p className="mt-1 text-sm text-muted">
              Atur pengalaman Ramadhan Anda.
            </p>

            {/* Hari Ramadhan */}
            <div className="mt-5 rounded-3xl border border-border bg-white p-4">
              <div className="text-sm font-semibold text-text">
                Hari ke-Ramadhan
              </div>
              <select
                value={ramadhanDay}
                onChange={(e) => setRamadhanDay(Number(e.target.value))}
                className="mt-3 w-full rounded-2xl border border-border bg-white px-4 py-3"
              >
                {Array.from({ length: 30 }).map((_, i) => (
                  <option key={i + 1} value={i + 1}>
                    Hari ke-{i + 1}
                  </option>
                ))}
              </select>
            </div>

            {/* Notifikasi */}
            <div className="mt-3 rounded-3xl border border-border bg-white p-4 space-y-3">
              <div className="text-sm font-semibold text-text">Notifikasi</div>

              <Toggle
                label="Imsak"
                active={notifImsak}
                onToggle={() => setNotifImsak((s) => !s)}
              />
              <Toggle
                label="Maghrib (Buka)"
                active={notifMaghrib}
                onToggle={() => setNotifMaghrib((s) => !s)}
              />

              <div className="text-xs text-muted">
                Notifikasi bersifat best-effort (aktif saat aplikasi terbuka).
              </div>
            </div>

            <button
              onClick={finish}
              disabled={loading}
              className="mt-6 w-full rounded-2xl px-4 py-3 text-white font-semibold disabled:opacity-60"
              style={{
                background:
                  "linear-gradient(135deg, rgb(16 185 129), rgb(59 130 246))",
              }}
            >
              {loading ? "Menyimpan..." : "Mulai"}
            </button>

            <button
              onClick={back}
              className="mt-3 w-full rounded-2xl border border-border bg-white px-4 py-3 text-sm font-semibold"
            >
              Kembali
            </button>
          </section>
        )}
      </div>
    </main>
  );
}

/* ---------- Toggle component ---------- */
function Toggle({
  label,
  active,
  onToggle,
}: {
  label: string;
  active: boolean;
  onToggle: () => void;
}) {
  return (
    <button
      onClick={onToggle}
      className="flex w-full items-center justify-between rounded-2xl border border-border bg-white px-4 py-3 active:scale-[0.98]"
    >
      <span className="text-sm font-medium text-text">{label}</span>
      <div className="h-8 w-14 rounded-full bg-border px-1">
        <div
          className="h-6 w-6 rounded-full transition"
          style={{
            transform: active ? "translateX(24px)" : "translateX(0)",
            background: active ? "rgb(var(--green))" : "#ccc",
          }}
        />
      </div>
    </button>
  );
}
